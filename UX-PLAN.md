...
| K23 | Structured checks (G2): the minimal cross-language record `{id, status pass\|fail\|skip, duration?, message?}` per build, from a *small* adapter set (junit.xml, `go test -json`, TAP) or emitted natively; **stable ids are the contract's heart** (adapters own name-stability for table-driven/parameterized tests) — they enable per-test history, new-vs-known failure split, and the flake board's data. Ingestion failure is a first-class state on the build ("report discarded, logs only" — G5), never silence. Richer report shapes stay artifacts (layer 3): stored and linkable, no history promises. **Scale rule**: suites reach ~8000 tests over ~100 packages — the UI's default view is failures flat + per-package roll-ups (count/time, slowest first, expand-on-demand) + a name filter; a flat all-tests list is never the default, and the API pages by package | Ph2–3 | logs + error-first excerpts (today's floor) |
| K24 | Measurements (benchmarks/G2-adjacent): `{id, value, unit, better: lower\|higher}` per build — values, not verdicts; the server renders generic deltas vs the job's last green and trends, and never decides that a regression fails a build (the emitting tool does). Distinct from K23 because pass/fail semantics don't fit numbers | Ph3–4 | show latest values with no baseline |
